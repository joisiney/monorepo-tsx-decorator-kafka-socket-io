...
export class ControllerComposer {
  constructor() {
    ...
    for (const { method, propertyKey, url: rawUrl, dto } of routes) {
      ...
      logger.push({ CONTROLLER: this.constructor.name, METHOD: method, LINK: url })
      app.route({
        method,
        url,
        handler: async (
          ...
          try {
            ...
            const errorService = new TransformExceptionService(this.constructor.name, url, method, error as Error)
