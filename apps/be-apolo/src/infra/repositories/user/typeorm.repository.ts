...
import {
  ...
  IOptional,
  IRequired,
  Injectable, Left, NotFoundException, PaginationEntity, Right
...
export class UserRepositoryTypeORM implements IUserRepository.Implements {
  ...
  async findAll(props: IUserRepository.InputFindAll): Promise<Either<PaginationEntity<UserEntity>, NotFoundException>> {
  ...
  async findByEmail(email: string): Promise<Either<UserEntity, NotFoundException>> {
  ...
  async create(props: Omit<IOptional<IUserDto, 'id'>, 'createdAt'>): Promise<Either<boolean, NotFoundException>> {
  ...
  async updateById({id, ...props}: IRequired<Partial<IUserDto>, 'id'>): Promise<Either<UserEntity, NotFoundException>> {
    const user = await this.user.findOne({ where: { id } })
    if (!user) {
      return new Left(new NotFoundException('User not found'))
    }
